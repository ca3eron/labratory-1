...
It stands for not applicable/available, the data is missing or doesn't exist.
...
The highest values in the data set must be extremely high (and thus be outlines) to be able to right-skew the mean to such an extent. I has a long right tail.
...
# Write your code here
 free_n_admission <- visitors_with_nations %>%
  mutate(
  admission = case_when(
    admission == "Free" ~ "Free", 
           TRUE ~ "Not Free")
...
free_n_admission %>%
  group_by(nation) %>% count(admission) %>% mutate(percent = (n / sum(n)) * 100)
...
visitors <- visitors %>% 
  mutate(changeper = (n_2022 - n_2021) / n_2021 * 100)

visitors %>% filter(region == "Scotland") %>% group_by(admission) 
## %>% summarise(mean_pct_change = mean(changeper, na.rm = TRUE)) %>%
  arrange(mean_pct_change) 

## GREEN IS CORRECTION WHICH I MISSED
